...
def test_generation_error_ignores_preexisting_alert_but_captures_new_error():
    ...
    result = analyze_generation_trace([
        snap(alerts=baseline),
        snap(disabled=True, alerts=baseline),
        snap(alerts=baseline + ["Tạo giọng thất bại. Vui lòng thử lại."]),
...
def test_reload_retry_only_for_explicit_reload_error():
    assert should_retry_after_reload(analyze_generation_trace([snap(), snap(alerts=["Không tải được giọng. Vui lòng tải lại trang."])])) is True
    assert should_retry_after_reload(analyze_generation_trace([snap(), snap(alerts=["Tạo giọng thất bại. Vui lòng thử lại."])])) is False


def test_network_summary_reports_only_safe_failure_metadata():
    result = _network_event_summary([
...
def test_safe_error_json_only_keeps_allowlisted_fields():
    assert _safe_error_json({
        "error": "forbidden", "message": "challenge failed", "token": "SECRET",
        "detail": {"code": "CF_BLOCK", "authorization": "SECRET3"},
    }) == {"error": "forbidden", "message": "challenge failed", "detail": {"code": "CF_BLOCK"}}
