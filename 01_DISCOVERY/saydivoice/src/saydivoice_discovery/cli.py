...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MAGASIN SaydiVoice Discovery Runner V0.4.3 (D3 controlled generation).")
    ...
    parser.add_argument("--allow-generate", action="store_true", help="Explicitly authorize one controlled D3 generation. May consume provider quota.")
    parser.add_argument("--retry-after-reload-error", action="store_true", help="Authorize one extra attempt only when the first new provider error explicitly asks to reload the page.")
